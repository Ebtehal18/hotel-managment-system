...
import { Breadcrumbs, CircularProgress, Link, Paper, useTheme } from '@mui/material';
...
onSuccess:(res)=>{
  ...
  return (
    ...
    <Box sx={{p:5,mt:3}}>
     ...
     <Typography sx={{textAlign:"center",color:theme.palette.text.disabled,fontSize:"26px",fontWeight:600}}>{t("user.Payment")}</Typography>
  ...
  >
      ...
      {activeStep === steps.length ? (
          ...
          <React.Fragment>
        {activeStep===0&&<AddressElement options={{ mode: "billing" }}/>}
        {activeStep===1&& <Box sx={{   border: "1px solid #ccc",
    borderRadius: "8px",
    ...
    margin: "auto",}}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
            ...
            <Box sx={{ flex: '1 1 auto' }} />
          {activeStep===0&&<Button variant='contained' onClick={handleNext}>
            {t('user.next')}
            </Button>}
            {activeStep===1&&<Button variant='contained' disabled={isPending}  onClick={submitPaymentHandler}>
                {isPending?<>
