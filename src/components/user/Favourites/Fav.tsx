import { Box, Breadcrumbs, Grid, Pagination, Typography, useTheme ,Link,Stack, IconButton} from "@mui/material";
import { useEffect, useState } from "react";
import { getDataFav, useGetFav } from "../../../hooks/useGetFav";
...
const navigate=useNavigate()
 ...
 <Box sx={{   
  ...
  "&:hover .overlay": {
   ...
   <Box component={'img'} className="image" src={room.images[0]??roomImg} sx={{width:"100%",borderRadius:"15px",height:215,objectFit:"cover",transition: "all 0.3s ease"}}/>
       <Box
                    sx={{
                      position: "absolute",
                      top: 0,
              ...
              >
                <IconButton sx={{p:0}} onClick={() => toggleFav(room._id)}>
                        {isFav(room._id)?<FavoriteIcon color="error" sx={{fontSize:32}}/>:
