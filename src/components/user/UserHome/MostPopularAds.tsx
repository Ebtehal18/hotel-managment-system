...
export default function MostPopularAds() {
  ...
  const BigAdSkeleton = () => (
    <Skeleton
      ...
      width="100%"
      height="100%"
      sx={{ borderRadius: "15px", minHeight: 500 }}
    />
  );
    ...
    <>
      ...
      <Grid container spacing={2}>
          ...
          {isLoading ? (
            ...
            >
              ...
              >
                <IconButton
                  sx={{ p: 0 }}
                  onClick={() => handelToggle(ads.data.ads[0].room._id)}
                >
                  {isFav(ads.data.ads[0].room._id) ? (
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Stack spacing={2}>
            {isLoading
              ? [1, 2].map((i) => <SmallAdSkeleton key={i} />)
              : [1, 2].map((index) => {
                  ...
                  return ad ? (
                    ...
                    >
                      ...
                      >
                        <IconButton
                          sx={{ p: 0 }}
                          onClick={() => handelToggle(ad.room._id)}
                        >
                          {isFav(ad.room._id) ? (
                            <FavoriteIcon color="error" sx={{ fontSize: 32 }} />
                          ) : (
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Stack spacing={2}>
            {isLoading
              ? [3, 4].map((i) => <SmallAdSkeleton key={i} />)
              : [3, 4].map((index) => {
                  const ad = ads?.data?.ads?.[index];
                  return ad ? (
                    <>
