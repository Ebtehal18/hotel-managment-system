...
import { getDataMyBooking, useGetMyBooking, type Booking } from '../../../hooks/useGetUserBooking'
...
import { Box ,Breadcrumbs,Chip, Typography, useTheme,Link, Grid, Card, CardContent } from '@mui/material'
...
export default function UserBooking() {
  ...
  const {data,isLoading}=useGetMyBooking(page)
  ...
  return data?.data.room.roomNumber ?? <BedIcon sx={{color:"primary.main"}} />;
};
    const columns: IColumn<Booking>[] = [
      { key: "room", label: t("user.room"),    render: (val) => <RoomCell roomId={val as string} />,
},
        { key: "startDate", label: t("user.startDate") ,render:(val)=>{
          const time =val as string
          return new Date(time).toLocaleDateString()
        }},
        { key: "endDate", label: t("user.endDate") ,render:(val)=>{
          const time =val as string
          return new Date(time).toLocaleDateString()
        }},
        { key: "totalPrice", label: t("user.totalPrice"),render:(val)=>{
          const price=val as string
          return `${price} ${t("user.egp")}`
        } },
                { key: "status", label: t("user.status"),render:(val)=>{
...
{key:'_id',label:t('user.payNow'),
 ...
 {data.status === "pending" ? (
  ...
  >
    ...
    //  "completed"
const totalBookings = data?.data.totalCount ?? 0;
const numOfPending = data?.data.myBooking.filter(b => b.status === "pending").length ?? 0;
const numOfCompleted = data?.data.myBooking.filter(b => b.status === "completed").length ?? 0;
...
const pendingPercentage = totalBookings > 0 ? (numOfPending / totalBookings) * 100 : 0;
const completedPercentage = totalBookings > 0 ? (numOfCompleted / totalBookings) * 100 : 0;
...
const cards = [
  {
    ...
    bg: "#1976d2",
    val: totalBookings,
    iconBg: "linear-gradient(135deg, rgba(25, 118, 210, 0.2) 0%, rgba(25, 118, 210, 0.1) 100%)",
  ...
  },
  {
    ...
    bg: "#ed6c02",
    val: numOfPending,
    iconBg: "linear-gradient(135deg, rgba(237, 108, 2, 0.2) 0%, rgba(237, 108, 2, 0.1) 100%)",
  ...
  },
  {
    ...
    bg: "#2e7d32",
    val: numOfCompleted,
    iconBg: "linear-gradient(135deg, rgba(46, 125, 50, 0.32) 0%, rgba(46, 125, 50, 0.1) 100%)",
  ...
  return (
    <>
    <Box sx={{p:5,mt:3}}>
  <Typography sx={{textAlign:"center",color:theme.palette.text.disabled,fontSize:"26px",fontWeight:600}}>{t("user.myBooking")}</Typography>
      ...
      >
        <CardContent>
          <Box display="flex" alignItems="center" gap={2}>
            <Box sx={{width:56,height:56,borderRadius:"8px",backgroundImage:card.iconBg,display:"flex",justifyContent:"center",alignItems:"center"}}>
              {card.icon}
            </Box>
